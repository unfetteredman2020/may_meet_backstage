<template>
  <div class="" style="background-color: #fff; height: 100%">
    <el-form style="background-color: #eee; padding: 10px 0 0" :rules="rules" :inline="true" :model="searchForm" ref="videoZoneSearchFormRef">
      ...
      <el-form-item>
        <el-button type="primary" @click="submitForm('videoZoneSearchFormRef')">查询</el-button>
        <el-button @click="resetForm('videoZoneSearchFormRef')">重置</el-button>
      </el-form-item>
    </el-form>
    <el-table :data="data" fit max-height="850px" border :header-cell-style="{ height: '20px', 'font-size': '12px', 'font-weight': '400', padding: '0!important', wordWrap: 'none;' }" stripe :row-style="{ height: '20px' }" :cell-style="{ padding: '0px', 'font-size': '12px', height: '20px', wordWrap: 'none' }">
      ...
      <el-table-column label="头像" prop="faceimg">
        <template slot-scope="scope">
          <el-image :preview-src-list="[BASE_CDN_DOMAIN + scope.row['faceimg']]" style="width: 40px; height: 40px; margin: 0 5px 0 0" :src="`${BASE_CDN_DOMAIN + scope.row['faceimg']}`" fit="fill"></el-image>
        </template>
      </el-table-column>
      <el-table-column label="封面" prop="imageurl">
        <template slot-scope="scope">
    ...
    <el-dialog title="视频审核" class="dialogs" :visible.sync="dialogFormVisible" :show-close="false" :close-on-click-modal="false">
      <div class="dialogBody">
        <div class="title">申请认证信息</div>
        <div class="voice">
          <p>提交语音签名：</p>
          <video :src="videoUrl" controls="controls" width="200" height="100">您的浏览器不支持播放该视频！</video>
        </div>
...
<script>
import { getVideoZoneList, changeVideoHandle } from "@/api/safeApi.js";
...
export default {
  ...
  methods: {
    ...
    edit(item) {
      ...
      this.videoUrl = `${this.BASE_CDN_DOMAIN}${item.videourl}`;
      (this.settingForm = {
        userid: item.userid,
    ...
    async changeStatus() {
      try {
        ...
        const res = await changeVideoHandle(params);
        console.log("changeVideoHandle res", res);
    ...
    async getData(data = {}) {
      try {
        const res = await getVideoZoneList(data);
        console.log("getVideoZoneList", res);
        if (res && res.errcode == 0) {
          this.data = res.data;
        } else {
