...
import { manRegistRetention } from "@/api/reportApi.js";
import { getDate } from "@/utils/date";
import tableConfig from "../tableConfig/zcnyhlcl_table_config"; // 公用头条 安卓新增付费用户table 配置
...
export default {
  ...
  data() {
    return {
      list: [],
      customProps: {
        tableName: "manRegistRetentionTableConfig", // tableName： table标识，用来做table的key
  ...
  methods: {
    async getTableData(data) {
      try {
        ...
        const res = await manRegistRetention(data || params);
        console.log("manRegistRetention res", res);
