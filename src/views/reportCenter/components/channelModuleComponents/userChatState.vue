<template>
  <div>
    <customSelect :text="'用户聊天情况'" :customConfig="customConfig"/>
...
import tableConfig from "../tableConfig/qd_yhltqk_table_config.js"; // 公用头条 安卓新增付费用户table 配置
import { userChatState, getMediaList, getLaunchCompany } from '@/api/reportApi.js'
...
export default {
  ...
  data() {
    return {
      customConfig: {
        isShowTransfer: false, // 是否显示穿梭框
      },
      list: [],
      customProps: {
        tableName: "userChatStateTableConfig", // tableName： table标识，用来做table的key
  ...
  methods: {
    async getData(time, obj={}) {
      try {
        ...
        const res = await userChatState(time)
        console.log('userChatState', res)
