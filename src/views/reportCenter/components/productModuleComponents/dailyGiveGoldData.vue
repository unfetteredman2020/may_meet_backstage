...
import { dailyGiveGoldData } from "@/api/reportApi.js";
import { getDate } from "@/utils/date";
import tableConfig from "../tableConfig/mrzsjbsj_table_config.js"; // 公用头条 安卓新增付费用户table 配置
...
export default {
  ...
  data() {
    return {
      list: [],
      customProps: {
        tableName: "dailyGiveGoldDataTableConfig", // tableName： table标识，用来做table的key
  ...
  methods: {
    async getTableData(data) {
      try {
        ...
        const res = await dailyGiveGoldData(data || params);
        console.log("dailyGiveGoldData res", res);
