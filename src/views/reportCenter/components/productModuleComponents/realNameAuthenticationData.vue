...
import { realNameAuthenticationData } from "@/api/reportApi.js";
import { getDate } from "@/utils/date";
import tableConfig from "../tableConfig/smzrrzsj_table_config.js"; // 公用头条 安卓新增付费用户table 配置
...
export default {
  ...
  data() {
    return {
      list: [],
      customProps: {
        tableName: "realNameAuthenticationDataTableConfig", // tableName： table标识，用来做table的key
  ...
  methods: {
    async getTableData(data) {
      try {
        ...
        const res = await realNameAuthenticationData(data || params);
        console.log("realNameAuthenticationData res", res);
